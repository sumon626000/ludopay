...
class RazorpayController extends Controller
{
    public function Initiate(Request $request)
    {
        ...
        $PayKey = Websetting::where("_id","60bed6aef3c80e44a06e01f0")->first();
    ...
    public function Complete(Request $request)
    {
        ...
        if ($signatureStatus == true) {
            ...
            if($insertTrans){
              $userData = Userdata::where("userid",$request->all()['userid'])->first();
              $prevAmount = $userData->points;
              $purchaseAmount =  $request->all()['amount']/100;
              $totalAmount = $prevAmount+$purchaseAmount;

              $updateCoin = Userdata::where("userid",$request->all()['userid'])->update(array(
                "points" => $totalAmount,
                ));
